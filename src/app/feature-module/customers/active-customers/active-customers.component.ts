...
export class ActiveCustomersComponent {
  public tableData: Array<activecustomer> = [];
  isCollapsed = false;
  ...
  public totalData = 0;
  dataSource!: MatTableDataSource<activecustomer>;
  ...
  constructor(
    ...
    this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
      if (this.router.url == this.routes.activecustomer) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getActive().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
      this.serialNumberArray = [];
      this.totalData = apiRes.totalData;
      apiRes.data.map((res: activecustomer, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<activecustomer>(this.tableData);
