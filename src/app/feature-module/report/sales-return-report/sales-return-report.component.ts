...
export class SalesReturnReportComponent {
  ...
  public tableData: Array<salesreturnreport> = [];
  // pagination variables
  public pageSize = 10;
  public serialNumberArray: Array<number> = [];
  public totalData = 0;
  dataSource!: MatTableDataSource<salesreturnreport>;
  ...
  constructor(
    ...
    this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
      if (this.router.url == this.routes.salesReturnReport) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getSalesReports().subscribe((apiRes: apiResultFormat) => {
      ...
      apiRes.data.map((res: salesreturnreport, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<salesreturnreport>(
        this.tableData
      );
